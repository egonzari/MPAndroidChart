...
public abstract class Utils {
  ...
  @SuppressWarnings("unused") public final static double DOUBLE_EPSILON =
      Double.longBitsToDouble(1);

  @SuppressWarnings("unused") public final static float FLOAT_EPSILON = Float.intBitsToFloat(1);

  /**
   * initialize method, called inside the Chart.init() method.
   */
  @SuppressWarnings("deprecation") public static void init(Context context) {
   ...
   */
  @Deprecated public static void init(Resources res) {
  ...
  public static float roundToNextSignificant(double number) {
    if (Double.isInfinite(number) || Double.isNaN(number) || number == 0.0) return 0;
  ...
  public static void drawImage(Canvas canvas, Drawable drawable, int x, int y, int width,
      int height) {
  ...
  public static void drawXAxisValue(Canvas c, String text, float x, float y, Paint paint,
  ...
  public static void drawXAxisImage(Canvas canvas, Bitmap drawable, float x, float y, Paint paint,
      MPPointF anchor, float angleDegrees) {
    ...
    final float lineHeight = paint.getFontMetrics(mFontMetricsBuffer);
   // paint.getTextBounds("ola que ase", 0, 11, mDrawTextRectBuffer);
    ...
    if (angleDegrees != 0.f) {
      ...
      //drawable.draw(canvas);
      canvas.drawBitmap(drawable, drawOffsetX, drawOffsetY, paint);
      canvas.restore();
    } else {
      if (anchor.x != 0.f || anchor.y != 0.f) {
      ...
      canvas.drawBitmap(drawable, drawOffsetX, drawOffsetY, null);
      //drawable.draw(canvas);
    }
  ...
  public static void drawMultilineText(Canvas c, StaticLayout textLayout, float x, float y,
      TextPaint paint, MPPointF anchor, float angleDegrees) {
  ...
  public static void drawMultilineText(Canvas c, String text, float x, float y, TextPaint paint,
      FSize constrainedToSize, MPPointF anchor, float angleDegrees) {

    StaticLayout textLayout = new StaticLayout(text, 0, text.length(), paint,
        (int) Math.max(Math.ceil(constrainedToSize.width), 1.f), Layout.Alignment.ALIGN_NORMAL, 1.f,
        0.f, false);
  ...
  public static FSize getSizeOfRotatedRectangleByRadians(float rectangleWidth,
    ...
    return FSize.getInstance(Math.abs(rectangleWidth * (float) Math.cos(radians)) + Math.abs(
        rectangleHeight * (float) Math.sin(radians)),
        Math.abs(rectangleWidth * (float) Math.sin(radians)) + Math.abs(
            rectangleHeight * (float) Math.cos(radians)));
  }
